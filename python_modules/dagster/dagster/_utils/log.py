...
def construct_single_handler_logger(name: str, level: Union[str, int], handler: logging.Handler) -> LoggerDefinition:
    check.str_param(name, "name")
...
def define_structured_logger(name: str, callback: Callable[[StructuredLoggerMessage], Any], level: Union[str, int]):
    check.str_param(name, "name")
